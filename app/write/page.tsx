    ...
    <div className="min-h-screen bg-[#FDFCFB] text-stone-900 font-serif">
      <nav className="flex justify-between items-center px-8 py-4 border-b border-stone-200 bg-white/80 sticky top-0 z-10 backdrop-blur-md">
        ...
        <div className="flex items-center gap-4">
          {isLoaded && user ? (
            <>
              <button onClick={() => handleSave(false)} disabled={isSaving} className="text-xs font-sans font-bold uppercase tracking-widest text-stone-400 hover:text-stone-900">
                {isSaving ? "SAVING..." : "Save Draft"}
              </button>
              <button onClick={() => handleSave(true)} disabled={isSaving} className="bg-stone-900 text-white px-6 py-2 rounded-full font-sans text-xs font-bold hover:bg-orange-700 transition-all">
